...
- (void)createMissingLabelsOrDeleteExtras {
    ...
    NSArray *increments = [ARHelpers incrementsForNumberOfItems:_totalNumberOfLabels range:NSMakeRange(0, _numberOfDataPoints - 1)];
    
    CGFloat stringWidth = [ARHelpers widthOfCaptionText:@"foo" inHeight:self.bounds.size.height];
    NSArray *xPositionIncrements = [ARHelpers incrementsForNumberOfItems:_totalNumberOfLabels range:NSMakeRange(0, self.bounds.size.width - stringWidth)];
...
- (void)updateAllLabelValues {
    ...
    NSArray *xPosiitonIncrements = [ARHelpers incrementsForNumberOfItems:_totalNumberOfLabels range:NSMakeRange(0, self.bounds.size.width - widthOfTestString)];
...
- (void)addTitleLabelConstraints {
    NSLayoutConstraint *bottomConstraint = [NSLayoutConstraint constraintWithItem:self attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.titleLabel attribute:NSLayoutAttributeBottom multiplier:1.0 constant:5.0];
